...
func (cl *Client) subscribe(params []interface{}, conf map[string]interface{},
	...
	req := newRequest(params, subscriptionMethod, conf, cl.shortID) // returns a request struct (payload)
...
// reply is passed by refference; msg is marshalled json
func decodeResponseFromMessage(msg []byte, reply interface{}) (err error) {
	...
	return json.Unmarshal(*resp.Params.Result, &reply) // returns reply and nil
}

func Connect(ctx context.Context, rpcEndpoint string) (client *Client, err error) {
...
func (cl *Client) handleSubscriptionMessage(subID uint64, message []byte) {
	...
	if err != nil {
		cl.closeSubscription(sub.req.ID, fmt.Errorf("Unable to decode client's response: %w", err))
		return
	}
